...
class BLEConstants:
    ...
    # IMU Config mappings
    ACCEL_GYRO_FREQ_MAP = {
        int(k): v for k, v in gatt_data["imu_config"]["accel_gyro_freq"].items()
    }
    MAG_FREQ_MAP = {int(k): v for k, v in gatt_data["imu_config"]["mag_freq"].items()}
    ACCEL_RANGE_MAP = {
        int(k): v for k, v in gatt_data["imu_config"]["accel_range"].items()
    }
    GYRO_RANGE_MAP = {
        int(k): v for k, v in gatt_data["imu_config"]["gyro_range"].items()
    }
