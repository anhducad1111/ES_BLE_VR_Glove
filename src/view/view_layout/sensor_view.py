...
class SensorView(ctk.CTkFrame):
    ...
    FLEX_SENSOR_CONFIG = SensorConfig(
        label_text="Flex Sensor (kOhm):",
        row=0,
        entry_count=5
    )
    
    FORCE_SENSOR_CONFIG = SensorConfig(
        label_text="Force Sensor (kOhm):",
        row=1,
        entry_count=1,
        pady=(20, 20)
    )
    ...
    def _create_sensor_section(self, parent: ctk.CTkFrame, config: SensorConfig) -> None:
    ...
    def _create_sensor_entries(self, parent: ctk.CTkFrame, config: SensorConfig) -> None:
    ...
    def _create_flex_entries(self, parent: ctk.CTkFrame, config: SensorConfig) -> None:
        ...
        for i in range(config.entry_count):
            sensor_num = i + 1
            entry = CoordinateEntry(parent, str(sensor_num), entry_width=config.entry_width)
    ...
    def _create_button_section(self) -> None:
        ...
        self.button_config = ButtonComponent(
            container, 
            "Configure", 
            command=self._handle_config_click
        )
