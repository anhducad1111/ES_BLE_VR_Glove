...
class FooterComponent(ctk.CTkFrame):
    ...
    def _create_footer(self):
        ...
        self.ble_status = ctk.CTkLabel(
            self, text="BLE OFF", font=self.config.LABEL_FONT, text_color="red"
        )
    ...
    def _update_time(self):
        ...
        if not self.is_synced and self.device_timestamp:
            try:
                ...
                if (
                    isinstance(self.device_timestamp, (int, float))
                    and 0 <= self.device_timestamp <= 32503680000
                ):  # Max timestamp for year 3000
                    ...
                    if time_diff > 5:
                        ...
                        text_color = (
                            "red" if self.blink_state else self.config.TEXT_COLOR
                        )
                else:
        ...
        self.timestamp_label.configure(
            text=display_time.strftime("%Y-%m-%d %H:%M:%S"), text_color=text_color
        )
